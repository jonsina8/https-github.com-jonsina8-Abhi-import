...
BOT_TOKEN = environ.get('BOT_TOKEN', "7768050831:AAHD39_p8uuIaS6TvLbhBMjh-66UOFvDUY4")
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6765929934 7167337214').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002559121988').split()]  # Channel id for auto indexing (make sure bot is admin)
...
DEENDAYAL_MOVIE_UPDATE_CHANNEL = int(environ.get('DEENDAYAL_MOVIE_UPDATE_CHANNEL', '-1002321973166'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002651565259'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '1002321973166')  # Channel/Group ID for force sub (make sure bot is admin)
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '1002641468018')  # Request channel id (make sure bot is admin)
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002321973166').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002321973166').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://kvivek40295:gE3TXza4xjGsk6al@cluster0.nztysg6.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/how_to_downlodm/14')  # Tutorial video link for opening shortlink website
